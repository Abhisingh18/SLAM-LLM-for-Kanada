...
def main(kwargs: DictConfig):
    ...
    # Resume: load optimizer state and fast-forward scheduler if resuming
    ckpt_path = kwargs.get("ckpt_path", None)
    resume_scheduler_steps = getattr(train_config, 'resume_scheduler_steps', 0)
    if ckpt_path is not None:
        ckpt_dir = os.path.dirname(ckpt_path)
        opt_state_path = os.path.join(ckpt_dir, "optimizer.pt")
        ...
        if resume_scheduler_steps > 0:
            logger.info(f"Fast-forwarding LR scheduler by {resume_scheduler_steps} steps")
            for _ in range(resume_scheduler_steps):
                scheduler.step()
            logger.info(f"LR after fast-forward: {scheduler.get_last_lr()[0]:.6f}")
