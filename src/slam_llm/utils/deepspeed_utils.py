...
def train(
    ...
    amp_dtype = torch.float16 if train_config.use_fp16 else torch.bfloat16
    ...
    for epoch in range(train_config.num_epochs):
        # Resume support: fast-forward past fully-completed epochs without
        # iterating their dataloaders at all.
        if epoch < resume_epoch:
            if rank == 0:
                logger.info(f"[resume] skipping completed epoch {epoch+1} (resume_epoch={resume_epoch})")
            continue
        dist.barrier()
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                if train_config.batching_strategy == "dynamic" and deepspeed_join(group_join):
                    break
                # Resume support: replay the dataloader past `resume_step` without
                # forward/backward so the DistributedSampler stays aligned with the
                # original run, then continue training from the checkpointed state.
                # Only fires on the epoch we resumed into (resume_epoch); earlier
                # epochs are skipped wholesale above, later epochs run normally.
                if epoch == resume_epoch and step < resume_step:
                    if rank == 0 and step % 500 == 0:
                        logger.info(f"[resume] skipping dataloader iter {step}/{resume_step} (epoch {epoch+1})")
                    pbar.update(1)
                    continue
                for key in batch.keys():
                    batch[key] = (
                        batch[key].to(local_rank).to(amp_dtype)
                ...
                cur_step = (epoch * total_length + step + 1 if train_config.batching_strategy != "dynamic" else step + 1)
                # checkpoint_interval decouples checkpoint-saving cadence from
                # eval cadence (validation_interval) -- e.g. eval only every
                # 10k steps (slow, full-dataset eval loop) while still saving
                # a checkpoint every 1k steps for crash-safety. Defaults to
                # validation_interval (old coupled behavior) if unset, so
                # existing scripts that don't pass it are unaffected.
                checkpoint_interval = getattr(train_config, "checkpoint_interval", train_config.validation_interval)

                if (
                    cur_step % train_config.validation_interval == 0
                ...
                elif train_config.save_model and cur_step % checkpoint_interval == 0:
                    # checkpoint-only tick (no eval this step)
                    checkpoint_name = f"{train_config.model_name}_epoch_{str(epoch+1)}_step_{step+1}"
                    save_model_checkpoint_deepspeed(
                        model, train_config, checkpoint_name
                    )
...
def evaluation(model, train_config, eval_dataloader, local_rank, tokenizer):
    ...
    eval_acc = 0.0
    autocast = torch.cuda.amp.autocast  # (Fix:MZY): fix expected scalar type mismatch in norm
    amp_dtype = torch.float16 if train_config.use_fp16 else torch.bfloat16

    # Optional cap on the number of eval batches per validation call, so eval
    # can run frequently (e.g. every checkpoint_interval) without paying the
    # cost of the full dev set each time. Unset/None = unlimited (old
    # behavior, evaluates the whole eval_dataloader).
    max_eval_steps = getattr(train_config, "max_eval_steps", None)
    ...
    with MemoryTrace() as memtrace:
        if train_config.batching_strategy != "dynamic":
            ...
            if max_eval_steps is not None:
                total_length = min(total_length, max_eval_steps)
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                ...
                with autocast(dtype=amp_dtype):  # (Fix:MZY): fix expected scalar type mismatch in norm
                    outputs, *rest = model(**batch)
                acc = rest[0] if rest else -1
                loss = outputs.loss
    ...
    # Compute average loss and perplexity. Divide by the number of batches
    # actually processed (respects max_eval_steps), not the full dataloader
    # length, so a capped eval still reports a correctly-averaged metric.
    if train_config.batching_strategy != "dynamic":
        n_processed = min(len(eval_dataloader), max_eval_steps) if max_eval_steps is not None else len(eval_dataloader)
    else:
        n_processed = step + 1
    eval_epoch_loss = eval_loss / n_processed
    eval_epoch_acc = eval_acc / n_processed
