...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, log_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    best_val_acc = 0.0
    resume_epoch = getattr(train_config, 'resume_epoch', 0)
    resume_step = getattr(train_config, 'resume_step', 0)
    for epoch in range(resume_epoch, train_config.num_epochs):
        epoch_start_time = time.perf_counter()
        join_ctx = Join([model]) if (train_config.enable_fsdp or train_config.enable_ddp) else nullcontext()
        with MemoryTrace() as memtrace, join_ctx:  # track the memory usage
            model.train()
            total_loss = 0.0
            total_acc = 0.0
            ...
            for step, batch in enumerate(train_dataloader):
                if epoch == resume_epoch and step < resume_step:
                    if (rank is None or rank == 0) and step % 5000 == 0:
                        logger.info(f"Skipping step {step}/{resume_step} (resuming)")
                    continue
