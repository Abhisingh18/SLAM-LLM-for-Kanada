#!/bin/bash
# Train: ESPnet2 Transformer ASR encoder (Nithya/Akshaya's checkpoint,
# 45M params, frozen) + linear projector + gemma-3-4b-it (LoRA)
# Bilingual Kannada-English code-switch experiment — encoder #3 of 5 in
# Prof. Umesh's encoder comparison study. Ported from Akshaya's setup
# (MULTI_Lang_1kfinetune-gemma-3-4b-it-transformer.sh) -- see
# src/slam_llm/models/espnet_transformer_encoder.py for the encoder wrapper.
# Uses OUR proven LoRA config (r=8/alpha=32) for apples-to-apples comparison
# with the other 4 encoders, not Akshaya's larger r=64/alpha=128.
...
# GPUs 6,7,8,9 -- NOTE: currently occupied by the Wav2Vec2-BERT training run
# (our own job). Re-check via nvidia-smi before launching; free them first
# (kill the w2vbert job) or this will fail/contend for memory.
...
speech_encoder_path=/speech/abhishek/kannada_data/encoders/transformer_espnet/23epoch.encoder_only.pth
...
output_dir=${OUTPUT_DIR:-/speech/abhishek/output/kannada-cs-transformer-gemma3-4b-finetuned}
mkdir -p "$output_dir"
...
++model_config.encoder_name=espnet_transformer \
++model_config.normalize=true \
++dataset_config.normalize=true \
++model_config.encoder_projector_ds_rate=5 \
++dataset_config.encoder_projector_ds_rate=5 \
++dataset_config.audio_frames_per_sec=31.25 \
...
++log_config.wandb_exp_name=espnet-transformer-kannada-bilingual-cs-gemma3-4b-it-lora \
