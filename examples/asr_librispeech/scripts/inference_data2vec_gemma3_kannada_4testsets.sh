...
for split in "${testsets[@]}"; do
    ...
    # Retry loop: the inference script has no internal error handling, and
    # this GPU cluster intermittently throws "CUDA error: an illegal
    # instruction was encountered" mid-generate() (a known recurring
    # GPU-level fault, same as during training). Without checking the exit
    # code, a crashed run would silently leave partial _pred/_gt files while
    # the loop still printed "Done". Retry up to 5x on nonzero exit.
    attempt=1
    max_attempts=5
