#!/bin/bash
# Train: data2vec-AQC encoder (multilingual PRETRAINED-ONLY, NOT Kannada
# fine-tuned, frozen) + linear projector + gemma-3-4b-it (LoRA)
# Bilingual Kannada-English code-switch experiment — encoder #2 of 5 in
# Prof. Umesh's encoder comparison study. Identical setup to
# finetune_data2vec_gemma3_kannada_cs.sh (encoder #1, Kannada fine-tuned)
# except for the encoder checkpoint + output_dir + wandb exp name.
...
# GPUs 1,2,3,4 — same as encoder #1's run, which has now been stopped
# (33 checkpoints saved, ~88% eval_acc). Re-check nvidia-smi before launching
# if other jobs may have started since.
...
output_dir=${OUTPUT_DIR:-/speech/abhishek/output/kannada-cs-data2vec-ssl-gemma3-4b-finetuned}
mkdir -p "$output_dir"
...
# AND all n_gpus rank optimizer-state shards under its global_step* subdir
# (partial/corrupt checkpoints from a crash mid-save are skipped). Override
# with RESUME_CKPT=... to force a specific checkpoint, or RESUME_CKPT=none to
# force a fresh run.
resume_ckpt=""
...
++log_config.wandb_exp_name=data2vec-aqc-ssl-pretrained-kannada-bilingual-cs-gemma3-4b-it-lora \
