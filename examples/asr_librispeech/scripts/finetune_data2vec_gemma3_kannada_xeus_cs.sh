#!/bin/bash
# Train: XEUS (CMU/ESPnet, 577M, 1M-hour pretrained-only, frozen) + linear
# projector + gemma-3-4b-it (LoRA)
# Bilingual Kannada-English code-switch experiment — encoder #4 of 5 in
...
# (name=xeus, dim=1024) + output_dir + wandb exp name.
#
# See src/slam_llm/models/xeus_encoder.py for the checkpoint-compat notes
# (older config.yaml schema patched at load time; ~1.5% of encoder params —
# one positional-embedding conv submodule — load randomly-initialized).
...
# GPUs 1,2,3,4 — same as encoders #1/#2's runs, both now stopped.
# Re-check nvidia-smi before launching, other users' jobs may have started since.
...
output_dir=${OUTPUT_DIR:-/speech/abhishek/output/kannada-cs-xeus-normed-gemma3-4b-finetuned}
mkdir -p "$output_dir"
...
++log_config.wandb_exp_name=xeus-normed-pretrained-kannada-bilingual-cs-gemma3-4b-it-lora \
